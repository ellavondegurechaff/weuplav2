...
export default async function handler(req, res) {
  ...
  try {
    // Try cache first
    const cachedProducts = await getFromCache(CACHE_KEYS.EDIBLE_PRODUCTS)
    if (cachedProducts) {
    ...
    // Set cache
    await setCache(CACHE_KEYS.EDIBLE_PRODUCTS, productsWithMedia)

    return res.status(200).json(productsWithMedia)
  } catch (error) {
    console.error('Database error:', error)
    return res.status(500).json({ message: 'Error fetching edible products' })
  }
} 