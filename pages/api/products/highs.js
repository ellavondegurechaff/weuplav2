...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cachedProducts = await getFromCache(CACHE_KEYS.HIGH_PRODUCTS)
    if (cachedProducts) {
    ...
    // Try to set cache but don't block on failure
    await setCache(CACHE_KEYS.HIGH_PRODUCTS, productsWithMedia)

    return res.status(200).json(productsWithMedia)
  } catch (error) {
    console.error('Database error:', error)
    return res.status(500).json({ message: 'Error fetching high-grade products' })
  }
} 